    ...
    <form onSubmit={handleSubmit} className="w-full" noValidate>
      ...
      >
        <div className="relative flex-1">
          <SearchIcon
            size={isLarge ? 18 : 16}
            className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--color-text-faint)]"
          />
          ...
          <input
            ...
            className={`w-full rounded-lg border bg-[var(--color-surface)] py-3 pl-10 pr-3 text-[var(--color-text)] placeholder:text-[var(--color-text-faint)] transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/40 ${
              isLarge ? 'text-base' : 'text-sm py-2.5'
            } ${error ? 'border-[var(--color-coral)]' : 'border-[var(--color-border-strong)]'}`}
          />
        </div>
        <button
          type="submit"
          disabled={isNavigating}
          className={`inline-flex items-center justify-center gap-2 rounded-lg bg-[var(--color-text)] font-medium text-[var(--color-bg)] transition-opacity hover:opacity-90 active:opacity-80 disabled:opacity-50 disabled:cursor-not-allowed ${
      ...
      {error ? (
        <p id={`${id}-error`} role="alert" className="mt-2 text-sm text-[var(--color-coral)]">
          {error}
        </p>
      ) : null}
    </form>
